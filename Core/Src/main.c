...
PUTCHAR_PROTOTYPE {
	HAL_StatusTypeDef status = HAL_UART_Transmit(&huart1, (uint8_t *)&ch, 1, HAL_MAX_DELAY);
  if (status != HAL_OK) {
    printf("\n");
  }
	return ch;
...
int main(void)
{
  ...
  USBPD_PreInitOs();
  /* USER CODE BEGIN 2 */
  my_printf(YELLOW("** restarted **\n"));
  HAL_GPIO_WritePin(UCPD_DBn_GPIO_Port, UCPD_DBn_Pin, GPIO_PIN_SET); // critical! UCPD_DBn high disconnects dead-battery resistors
  /* USER CODE END 2 */
...
void Error_Handler(void)
{
  ...
  my_printf(RED_BG("error handler") "\n");
